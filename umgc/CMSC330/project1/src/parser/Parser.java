...
import src.Scene;
import src.Text;
import src.polygons.hollow.Rectangle;
import src.polygons.hollow.RightTriangle;
import src.polygons.solid.IsoscelesTriangle;
import src.polygons.solid.Parallelogram;
import src.polygons.solid.RegularPolygon;
 ...
 * @description Class Parser: Parses the scene definition file and creates a graphical scene based
...
public class Parser {
  ...
  private void parseImages(Scene scene, Token imageToken)
    ...
    if (imageToken == Token.RIGHT_TRIANGLE) {
      ...
      Parallelogram parallelogram = new Parallelogram(color, new Point(location[0], location[1]),
          new Point(location2[0], location2[1]), offset);
      ...
      RegularPolygon regularPolygon =
          new RegularPolygon(color, new Point(location[0], location[1]), sides, radius);
      ...
      verifyNextToken(Token.NUMBER);
      width = lexer.getNumber();
      IsoscelesTriangle isosceles =
          new IsoscelesTriangle(color, new Point(location[0], location[1]), height, width);
      scene.addImage(isosceles);
    } else if (imageToken == Token.TEXT) {
      verifyNextToken(Token.STRING);
      String text = lexer.getString();
      Text textImage = new Text(color, new Point(location[0], location[1]), text);
